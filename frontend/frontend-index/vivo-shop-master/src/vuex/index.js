...
const mutations = {
  ...
  [DEL_ADDRESS](state, index) {
    let addressNumber = state.address.length;
    let totalMessage = "";
    if (addressNumber > 1) {
      totalMessage =
        "是否删除" +
        state.address[index].name +
        "?,<br />删除后将自动把后一条地址设置为默认";
    } else {
      totalMessage = "是否删除" + state.address[index].name + "?";
    }
    MessageBox({
      title: "提示",
